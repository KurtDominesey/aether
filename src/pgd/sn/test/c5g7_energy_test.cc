...
class C5G7EnergyTest : public ::testing::Test {
 protected:
  ...
  void SetUp() override {
    ...
    const std::vector<std::string> materials = {"water"}; //{"water", "uo2"};
    AssertDimension(materials.size(), num_mats);
    const int num_groups = 7;
    const int num_ell = 1;
    // const Mgxs mgxs = read_mgxs(filename, temperature, materials);
    const Mgxs mgxs(num_groups, materials.size(), num_ell);
    const Mgxs mgxs_one(1, materials.size(), num_ell);
    Mgxs mgxs_pseudo(1, materials.size(), num_ell);
    // Create quadrature
    const QPglc<dim, qdim> quadrature(4, 4);
    // Create mesh
    ...
    // dealii::GridTools::rotate(-dealii::numbers::PI_2, mesh);
    ...
    //         at_diag &= (face->vertex(v)[0] == face->vertex(v)[1]);
    //       if (at_diag)
    ...
    for (int cycle = 0; cycle < num_cycles; ++cycle) {
      ...
      pgd::sn::NonlinearGS nonlinear_gs(
          linear_ops, materials.size(), num_ell, num_sources);
      nonlinear_gs.enrich();
      for (int k = 0; k < 20; ++k) {
        energy_mg.modes.back() /= energy_mg.modes.back().l2_norm();
        nonlinear_gs.step(dealii::BlockVector<double>(),
                          dealii::BlockVector<double>());
      }
      double scaling = energy_mg.modes.back()[0];
      ...
      for (int n = 0; n < quadrature.size(); ++n) {
        ...
        std::cout << "n" << n << " " << l2_error << std::endl;
