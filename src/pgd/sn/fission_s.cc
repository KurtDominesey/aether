...
void FissionS<dim, qdim>::vmult(dealii::BlockVector<double> &dst,
  ...
  dealii::BlockVector<double> emitted_d(num_groups, num_ordinates*num_dofs);
  ...
  for (int m = 0; m < num_modes; ++m) {
    ...
    for (int mp = 0; mp < num_modes; ++mp) {
      ...
      emission[m][mp].vmult_add(emitted, produced);
    }
    m2d.vmult(emitted_d, emitted);
    for (int g = 0; g < num_groups; ++g) {
      dst.block(m*num_groups+g) = emitted_d.block(g);
    }
  }
}
