...
namespace internal
{
  namespace LAPACKFullMatrix_Implementation
  {
...
LAPACKFullMatrix_<number>::LAPACKFullMatrix_(const size_type n)
...
template <typename number>
LAPACKFullMatrix_<number>::LAPACKFullMatrix_(const size_type m, const size_type n)
...
template <typename number>
LAPACKFullMatrix_<number> &
LAPACKFullMatrix_<number>::operator=(const LAPACKFullMatrix_<number> &M)
{
...
template <typename number>
void
LAPACKFullMatrix_<number>::reinit(const size_type n)
{
...
template <typename number>
void
LAPACKFullMatrix_<number>::grow_or_shrink(const size_type n)
{
...
template <typename number>
void
LAPACKFullMatrix_<number>::apply_givens_rotation(
...
template <typename number>
void
LAPACKFullMatrix_<number>::remove_row_and_column(const size_type row,
                                                const size_type col)
{
...
template <typename number>
void
LAPACKFullMatrix_<number>::reinit(const size_type m, const size_type n)
{
...
LAPACKFullMatrix_<number> &
LAPACKFullMatrix_<number>::operator=(const FullMatrix<number2> &M)
{
...
template <typename number2>
LAPACKFullMatrix_<number> &
LAPACKFullMatrix_<number>::operator=(const SparseMatrix<number2> &M)
{
...
template <typename number>
LAPACKFullMatrix_<number> &
LAPACKFullMatrix_<number>::operator=(const number d)
{
  (void)d;
...
template <typename number>
LAPACKFullMatrix_<number> &
LAPACKFullMatrix_<number>::operator*=(const number factor)
{
...
template <typename number>
void
LAPACKFullMatrix_<number>::add(const number a, const LAPACKFullMatrix_<number> &A)
{
  Assert(state == LAPACKSupport::matrix ||
           state == LAPACKSupport::inverse_matrix,
         ExcState(state));
  ...
  // LAPACKFullMatrix_ is stored in contiguous array
...
namespace
{
  template <typename number>
  void
  cholesky_rank1(LAPACKFullMatrix_<number> &A,
                 const number              a,
                 const Vector<number> &    v)
  {
    const typename LAPACKFullMatrix_<number>::size_type N = A.n();
    Vector<number>                                     z(v);
    ...
    if (a > 0.)
      {
        ...
        z *= std::sqrt(a);
        for (typename LAPACKFullMatrix_<number>::size_type k = 0; k < N; ++k)
          {
            const std::array<number, 3> csr =
              Utilities::LinearAlgebra::givens_rotation(A(k, k), z(k));
            A(k, k) = csr[2];
            for (typename LAPACKFullMatrix_<number>::size_type i = k + 1; i < N;
                 ++i)
              {
  ...
  template <typename number>
  void
  cholesky_rank1(LAPACKFullMatrix_<std::complex<number>> & /*A*/,
...
template <typename number>
void
LAPACKFullMatrix_<number>::rank1_update(const number a, const Vector<number> &v)
{
...
template <typename number>
void
LAPACKFullMatrix_<number>::vmult(Vector<number> &      w,
                                const Vector<number> &v,
...
template <typename number>
void
LAPACKFullMatrix_<number>::Tvmult(Vector<number> &      w,
                                 const Vector<number> &v,
                                 const bool            adding) const
{
  const types::blas_int mm    = this->m();
...
template <typename number>
void
LAPACKFullMatrix_<number>::vmult_add(Vector<number> &      w,
...
template <typename number>
void
LAPACKFullMatrix_<number>::Tvmult_add(Vector<number> &      w,
                                     const Vector<number> &v) const
{
  Tvmult(w, v, true);
}
...
template <typename number>
void
LAPACKFullMatrix_<number>::mmult(LAPACKFullMatrix_<number> &      C,
                                const LAPACKFullMatrix_<number> &B,
                                const bool                      adding) const
{
...
template <typename number>
void
LAPACKFullMatrix_<number>::mmult(FullMatrix<number> &            C,
                                const LAPACKFullMatrix_<number> &B,
                                const bool                      adding) const
{
  Assert(state == matrix || state == inverse_matrix, ExcState(state));
...
template <typename number>
void
LAPACKFullMatrix_<number>::Tmmult(LAPACKFullMatrix_<number> &      C,
                                 const LAPACKFullMatrix_<number> &B,
                                 const Vector<number> &          V,
                                 const bool                      adding) const
{
  ...
  const LAPACKFullMatrix_<number> &A = *this;
...
template <typename number>
void
LAPACKFullMatrix_<number>::transpose(LAPACKFullMatrix_<number> &B) const
{
  const LAPACKFullMatrix_<number> &A = *this;
...
template <typename number>
void
LAPACKFullMatrix_<number>::scale_rows(const Vector<number> &V)
{
  LAPACKFullMatrix_<number> &A = *this;
  Assert(state == matrix || state == inverse_matrix, ExcState(state));
  Assert(V.size() == A.m(), ExcDimensionMismatch(A.m(), V.size()));
...
template <typename number>
void
LAPACKFullMatrix_<number>::Tmmult(FullMatrix<number> &            C,
                                 const LAPACKFullMatrix_<number> &B,
                                 const bool                      adding) const
{
  Assert(state == matrix || state == inverse_matrix, ExcState(state));
...
template <typename number>
void
LAPACKFullMatrix_<number>::mTmult(LAPACKFullMatrix_<number> &      C,
                                 const LAPACKFullMatrix_<number> &B,
                                 const bool                      adding) const
{
  Assert(state == matrix || state == inverse_matrix, ExcState(state));
...
template <typename number>
void
LAPACKFullMatrix_<number>::mTmult(FullMatrix<number> &            C,
                                 const LAPACKFullMatrix_<number> &B,
                                 const bool                      adding) const
{
  Assert(state == matrix || state == inverse_matrix, ExcState(state));
...
template <typename number>
void
LAPACKFullMatrix_<number>::TmTmult(LAPACKFullMatrix_<number> &      C,
                                  const LAPACKFullMatrix_<number> &B,
                                  const bool                      adding) const
{
  Assert(state == matrix || state == inverse_matrix, ExcState(state));
...
template <typename number>
void
LAPACKFullMatrix_<number>::TmTmult(FullMatrix<number> &            C,
                                  const LAPACKFullMatrix_<number> &B,
                                  const bool                      adding) const
{
  Assert(state == matrix || state == inverse_matrix, ExcState(state));
...
template <typename number>
void
LAPACKFullMatrix_<number>::compute_lu_factorization()
{
...
template <typename number>
void
LAPACKFullMatrix_<number>::set_property(const Property p)
{
  property = p;
}
...
template <typename number>
number
LAPACKFullMatrix_<number>::l1_norm() const
{
  const char type('O');
  return norm(type);
}
...
template <typename number>
number
LAPACKFullMatrix_<number>::linfty_norm() const
{
  const char type('I');
  return norm(type);
}
...
template <typename number>
number
LAPACKFullMatrix_<number>::frobenius_norm() const
{
  const char type('F');
  return norm(type);
}
...
template <typename number>
number
LAPACKFullMatrix_<number>::norm(const char type) const
{
  std::lock_guard<std::mutex> lock(mutex);
...
template <typename number>
number
LAPACKFullMatrix_<number>::trace() const
{
  Assert(state == LAPACKSupport::matrix ||
           state == LAPACKSupport::inverse_matrix,
...
template <typename number>
void
LAPACKFullMatrix_<number>::compute_cholesky_factorization()
{
  Assert(state == matrix, ExcState(state));
...
template <typename number>
number
LAPACKFullMatrix_<number>::reciprocal_condition_number(const number a_norm) const
{
  std::lock_guard<std::mutex> lock(mutex);
...
template <typename number>
number
LAPACKFullMatrix_<number>::reciprocal_condition_number() const
{
  std::lock_guard<std::mutex> lock(mutex);
...
template <typename number>
void
LAPACKFullMatrix_<number>::compute_inverse_svd(const double threshold)
{
  if (state == LAPACKSupport::matrix)
    compute_svd();
...
template <typename number>
void
LAPACKFullMatrix_<number>::compute_inverse_svd_with_kernel(
...
template <typename number>
void
LAPACKFullMatrix_<number>::invert()
{
...
template <typename number>
void
LAPACKFullMatrix_<number>::solve(Vector<number> &v, const bool transposed) const
{
...
template <typename number>
void
LAPACKFullMatrix_<number>::solve(LAPACKFullMatrix_<number> &B,
...
template <typename number>
void
LAPACKFullMatrix_<number>::apply_lu_factorization(Vector<number> &v,
                                                 const bool transposed) const
{
  solve(v, transposed);
}
...
template <typename number>
void
LAPACKFullMatrix_<number>::apply_lu_factorization(LAPACKFullMatrix_<number> &B,
                                                 const bool transposed) const
{
  solve(B, transposed);
}
...
template <typename number>
number
LAPACKFullMatrix_<number>::determinant() const
{
  Assert(this->m() == this->n(), LACExceptions::ExcNotQuadratic());
...
template <typename number>
void
LAPACKFullMatrix_<number>::compute_eigenvalues(const bool right, const bool left)
{
  Assert(state == matrix, ExcState(state));
  const types::blas_int nn = this->n();
  wr.resize(nn);
  ...
  internal::LAPACKFullMatrix_Implementation::geev_helper(jobvl,
                                                        jobvr,
                                                        this->values,
                                                        this->m(),
                                                        wr,
...
template <typename number>
void
LAPACKFullMatrix_<number>::compute_eigenvalues_symmetric(
  ...
  FullMatrix<number> &eigenvectors)
{
  ...
  wr.resize(nn);
  LAPACKFullMatrix_<number> matrix_eigenvectors(nn, nn);
...
template <typename number>
void
LAPACKFullMatrix_<number>::compute_generalized_eigenvalues_symmetric(
  LAPACKFullMatrix_<number> &   B,
  const number                 lower_bound,
  const number                 upper_bound,
  const number                 abs_accuracy,
  Vector<number> &             eigenvalues,
...
template <typename number>
void
LAPACKFullMatrix_<number>::print_formatted(std::ostream &     out,
...
template <typename number>
void
PreconditionLU_<number>::initialize(const LAPACKFullMatrix_<number> &M)
{
  matrix = &M;
  mem    = nullptr;
}
...
template <typename number>
void
PreconditionLU_<number>::vmult(Vector<number> &      dst,
...
template <typename number>
void
PreconditionLU_<number>::Tvmult(Vector<number> &      dst,
                               const Vector<number> &src) const
{
  dst = src;
...
template <typename number>
void
PreconditionLU_<number>::vmult(BlockVector<number> &      dst,
...
template <typename number>
void
PreconditionLU_<number>::Tvmult(BlockVector<number> &      dst,
                               const BlockVector<number> &src) const
{
  Assert(mem != nullptr, ExcNotInitialized());
  Vector<number> *aux = mem->alloc();
