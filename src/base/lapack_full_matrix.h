 ...
 */
template <typename number>
class LAPACKFullMatrix_ : public TransposeTable<number>
{
public:
  /**
   ...
   */
  explicit LAPACKFullMatrix_(const size_type size = 0);
   ...
   * \times \rm{cols}$.
   */
  LAPACKFullMatrix_(const size_type rows, const size_type cols);
   ...
   */
  LAPACKFullMatrix_(const LAPACKFullMatrix_ &);

  /**
   * Assignment operator.
   */
  LAPACKFullMatrix_<number> &
  operator=(const LAPACKFullMatrix_<number> &);
   ...
   */
  void
  add(const number a, const LAPACKFullMatrix_<number> &B);
   ...
   */
  void
  mmult(LAPACKFullMatrix_<number> &      C,
   ...
   */
  void
  Tmmult(LAPACKFullMatrix_<number> &      C,
         const LAPACKFullMatrix_<number> &B,
         const bool                      adding = false) const;
   ...
   */
  void
  mTmult(LAPACKFullMatrix_<number> &      C,
         const LAPACKFullMatrix_<number> &B,
         const bool                      adding = false) const;
   ...
   */
  void
  TmTmult(LAPACKFullMatrix_<number> &      C,
          const LAPACKFullMatrix_<number> &B,
          const bool                      adding = false) const;
   ...
   */
  void
  transpose(LAPACKFullMatrix_<number> &B) const;
   ...
   */
  void
  solve(LAPACKFullMatrix_<number> &B, const bool transposed = false) const;
  ...
  DEAL_II_DEPRECATED
  void
  apply_lu_factorization(LAPACKFullMatrix_<number> &B,
   ...
   * called.
   */
  inline const LAPACKFullMatrix_<number> &
  get_svd_u() const;
...
private:
   ...
   */
  std::unique_ptr<LAPACKFullMatrix_<number>> svd_u;

  /**
   * The matrix $\mathbf V^T$  in the singular value decomposition
   * $\mathbf U \cdot \mathbf S \cdot \mathbf V^T$.
   */
  std::unique_ptr<LAPACKFullMatrix_<number>> svd_vt;
...
/**
 * A preconditioner based on the LU-factorization of LAPACKFullMatrix_.
 *
 * @ingroup Preconditioners
 * @author Guido Kanschat, 2006
 */
template <typename number>
class PreconditionLU_ : public Subscriptor
{
public:
  void
  initialize(const LAPACKFullMatrix_<number> &);
  void
  initialize(const LAPACKFullMatrix_<number> &, VectorMemory<Vector<number>> &);
  void
...
private:
  SmartPointer<const LAPACKFullMatrix_<number>, PreconditionLU_<number>> matrix;
  SmartPointer<VectorMemory<Vector<number>>, PreconditionLU_<number>>   mem;
};
...
template <typename number>
inline void
LAPACKFullMatrix_<number>::set(const size_type i,
                              const size_type j,
                              const number    value)
{
  (*this)(i, j) = value;
...
template <typename number>
inline typename LAPACKFullMatrix_<number>::size_type
LAPACKFullMatrix_<number>::m() const
{
...
template <typename number>
inline typename LAPACKFullMatrix_<number>::size_type
LAPACKFullMatrix_<number>::n() const
{
...
template <typename MatrixType>
inline void
LAPACKFullMatrix_<number>::copy_from(const MatrixType &M)
{
  this->reinit(M.m(), M.n());
...
template <typename MatrixType>
inline void
LAPACKFullMatrix_<number>::fill(const MatrixType &M,
...
template <typename number2>
void
LAPACKFullMatrix_<number>::vmult(Vector<number2> &,
                                const Vector<number2> &,
                                const bool) const
{
  Assert(false,
         ExcMessage("LAPACKFullMatrix_<number>::vmult must be called with a "
...
template <typename number2>
void
LAPACKFullMatrix_<number>::vmult_add(Vector<number2> &,
                                    const Vector<number2> &) const
{
  Assert(false,
         ExcMessage("LAPACKFullMatrix_<number>::vmult_add must be called with a "
                    "matching Vector<double> vector type."));
}
...
template <typename number2>
void
LAPACKFullMatrix_<number>::Tvmult(Vector<number2> &,
                                 const Vector<number2> &,
                                 const bool) const
{
  Assert(false,
         ExcMessage("LAPACKFullMatrix_<number>::Tvmult must be called with a "
                    "matching Vector<double> vector type."));
}
...
template <typename number2>
void
LAPACKFullMatrix_<number>::Tvmult_add(Vector<number2> &,
                                     const Vector<number2> &) const
{
  Assert(false,
         ExcMessage(
           "LAPACKFullMatrix_<number>::Tvmult_add must be called with a "
           "matching Vector<double> vector type."));
}
...
LAPACKFullMatrix_<number>::eigenvalue(const size_type i) const
{
...
template <typename number>
inline number
LAPACKFullMatrix_<number>::singular_value(const size_type i) const
{
...
template <typename number>
inline const LAPACKFullMatrix_<number> &
LAPACKFullMatrix_<number>::get_svd_u() const
{
  Assert(state == LAPACKSupport::svd || state == LAPACKSupport::inverse_svd,
         LAPACKSupport::ExcState(state));
...
template <typename number>
inline const LAPACKFullMatrix_<number> &
LAPACKFullMatrix_<number>::get_svd_vt() const
{
  Assert(state == LAPACKSupport::svd || state == LAPACKSupport::inverse_svd,
         LAPACKSupport::ExcState(state));
