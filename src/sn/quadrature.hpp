...
template <int dim>
dealii::Tensor<1, dim> ordinate(const dealii::Point<2> coordinate) {
  Assert(2 <= dim && dim <= 3, dealii::ExcMessage("2D or 3D only"))
...
dealii::Tensor<1, 1> ordinate(const dealii::Point<1> coordinate) {
  return coordinate;
}
